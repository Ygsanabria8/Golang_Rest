...
func CreateFollow(w http.ResponseWriter, r *http.Request) {
	...
	status, err := repository.CreateFollow(follow)
	if err != nil {
		http.Error(w, "Error creating follow"+err.Error(), http.StatusBadRequest)
		return
	}

	if !status {
		http.Error(w, "Cannot create follow"+err.Error(), http.StatusBadRequest)
		return
	}
